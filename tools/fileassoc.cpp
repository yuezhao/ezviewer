               ...
               const QString &friendlyName)
{
    ...
    if(!typeDescription.isNull())
        Reg.setValue(classId + REG_DEFAULT, typeDescription);/// txt格式如果没有这一句，将无法新建文本文档。
    ...
    Reg.setValue(classId + "/DefaultIcon/.", QString("\"%1\",0").arg(appPath)); /// 将当前运行的程序资源中的第一个图标置入
...
bool clearAssociation(const QString & extension)
{
    ...
    if (QSysInfo::WindowsVersion >= QSysInfo::WV_NT){
        if (RegCU.value("Software/Classes/" + ext + REG_DEFAULT).toString() == ProgID) //Only remove if we own it
            /// MSDN上不建议删除，可能会引发其他问题。例如删除.txt组时导致无法新建文本文档。
